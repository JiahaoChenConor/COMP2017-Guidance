...
void print_inventory_action(bool is_product,unsigned int id, int item_id){
    if(is_product)
...
void* product(void *id){
    ...
    while(true){
        ...
        print_inventory_action(true,cur_p_id+1,(product_index%ITEM_NUM) + 1);
        product_index++;
...
void* consume(void *id){
    ...
    while(true){
        ...
        print_inventory_action(false,cur_p_id+1,(consume_index%ITEM_NUM) + 1);
        consume_index++;
        pthread_mutex_unlock(&mutex);
