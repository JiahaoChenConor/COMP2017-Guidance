/*
    ...
    2. Why doesn’t wrapping a mutex to protect alice_has_mail and bob_has_mail work?
    Hint: what happens to a pthread_cond_signal when no thread is waiting on it?
    ...
    |       while(!alice_has_) |                       |
    |        a=false, b=true   |    
    |   stop at first line loop|                       |

    |                          |    since b=ture,skip loop
    |                          |    change into a=ture, b=false


    |                          |    send signal(lost, since alice has not wait)|


    |   pthread_cond_wait      |
    |   (blocking here waiting |
    |    for signal)           |                                                  
    |                                 
                                | enter the loop. since b=false
                                |wait for alice signal. (dead lock)
...
void* alice(void* arg) {
    for (unsigned i = 0; i < EXCHANGES; ++i) {
        ...
        while (!alice_has_mail) {
            // 1. alice_has_mail = false. wait bob
