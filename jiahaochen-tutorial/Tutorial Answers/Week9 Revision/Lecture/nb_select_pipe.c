...
void child(int readfd) {
	...
	while (1) 
	{
		...
		FD_ZERO(&allfds);
		FD_SET(readfd, &allfds); // pipe read side
...
int main(void)
{
	...
	// 0,1,2 automatically available/create
	...
	if (0 == pid) { // child
		close(0);
		close(2);
		close(notify[1]); // writing end is closed
		child(notify[0]);
		close(notify[0]); // after finished, close the read end of the pipe
	} else { // parent
		close(2);
		close(notify[0]);
		parent(notify[1]);
